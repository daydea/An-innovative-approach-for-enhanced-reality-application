...
from tensorflow.keras.layers import Layer, Conv2D, AveragePooling2D, ReLU, Multiply, Dense, Activation, GlobalAveragePooling2D,Reshape, Multiply, Activation, MaxPool2D
...
#空间注意力机制1
# TensorFlow 版本的 SpatialAttention 类
class SpatialAttention(tf.keras.layers.Layer):
    def __init__(self, channels):
        super(SpatialAttention, self).__init__()
        # 使用与输入相同的通道数初始化卷积层
        self.conv = tf.keras.layers.Conv2D(filters=channels, kernel_size=1, padding='same', use_bias=False)
        self.sigmoid = tf.keras.layers.Activation('sigmoid')
        self.attention_weights = None
        
    def call(self, inputs):
        # 使用 keepdims=True 确保 avgout 和 maxout 保持相同的空间维度
        avgout = tf.keras.backend.mean(inputs, axis=-1, keepdims=True)
        maxout = tf.keras.backend.max(inputs, axis=-1, keepdims=True)
        
        # 将 avgout 和 maxout 合并，得到 [batch_size, height, width, 2] 形状的张量
        x1 = tf.keras.layers.concatenate([avgout, maxout], axis=-1)
        
        # 使用 Conv2D 层计算注意力权重，输出通道数与输入相同
        x1 = self.conv(x1)
        
        # 使用 Sigmoid 激活函数
        x1 = self.sigmoid(x1)
        self.attention_weights = x1 # 存储注意力权重
        # 确保 x1 的形状与 inputs 相匹配，以便可以进行逐元素相乘
        # 如果需要，可以使用 squeeze 来移除单维度，但通常这不是必要的
        # x1 = tf.keras.layers.squeeze(x1, axis=-1) 如果需要的话
...
def _se_block(inputs, filters, prefix, se_ratio=1 / 4.):
    ...
    # 添加空间注意力机制
    attention_layer = SpatialAttention(channels=filters)  # 使用文件中定义的类创建实例
    inputs, attention_weights = attention_layer(inputs)  # 使用注意力层实例处理输入
...
# 创建特征提取模型
def create_feature_extraction_model(model):
    inputs = model.inputs
    # 收集所有SpatialAttention层的输出
    attention_layers = [layer for layer in model.layers if isinstance(layer, SpatialAttention)]
    outputs = [layer.output for layer in attention_layers]
    # 创建新的模型，输出特定层的激活和注意力权重
    feature_extraction_model = Model(inputs, outputs)
...
def mobilenet_v3_small(input_shape=(224, 224, 3),
    ...
    # 添加 Dropout 层
    dropout_rate = 0.5  # Dropout 率，可以根据需要调整
    ...
    model = Model(img_input, x, name="MobilenetV3small")
